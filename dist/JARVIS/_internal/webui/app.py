...
async def startup_event():
    ...
    try:
        ...
        # Start the background ingestion loop as a non-blocking asyncio task
        ...
        logger.info("✅ JARVIS Orchestrator & Continuous Learner initialized")
    except Exception as e:
...
async def shutdown_event():
    ...
    # Gracefully stop JARVIS background learner
...
async def chat_stream_endpoint(request: ChatRequest):
    ...
    async def jarvis_generator():
        """Uses the JARVIS Orchestrator pipeline: Plan → Memory → Execute → Stream → Learn."""
        global _jarvis_orchestrator
        async for chunk in _jarvis_orchestrator.process(request.prompt):
        ...
        try:
            ...
            logger.warning(f"Failed to log JARVIS response: {e}")
    ...
    # Use JARVIS if ready, else fall back
