...
async function fetchWithRetry(url, options = {}, retries = 3, backoff = 1000) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.warn(`Connection issue. Retrying ${url} in ${backoff}ms...`);
...
async function checkHealth() {
  try {
    ...
    if (res.ok) {
      document.getElementById('stStatus').innerHTML = '<span class="dot"></span>Online';
      document.getElementById('stStatus').style.color = 'var(--green)';
    }
  } catch (e) {
    document.getElementById('stStatus').innerHTML = '<span class="dot" style="background:var(--red)"></span>Offline';
...
async function sendChat() {
  ...
  try {
    if (ac) {
      ...
      const res = await fetchWithRetry(API + '/chat/stream', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: abortCtl.signal });
      ...
      outer: while (true) {
        ...
        for (const line of dec.decode(value).split('\\n')) {
          ...
          try { 
            ...
            if (dd.done) break outer; 
            if (dd.token) { 
              let t = dd.token;
              if (t.includes("Observation:") || t.includes("JARVIS:")) continue;
              
              full += t; 
              toks++; 
              const tps = (toks / ((Date.now() - t0) / 1000)).toFixed(1); 
              document.getElementById('spdVal').textContent = tps; 
              document.getElementById('spdBadge').classList.add('on'); 
              
              let displayHtml = renderMd(full)
                .replace(/\\*\\s*\\(Executing Tool: (.*?)\\)\\s*\\*/g, '<div class="fbadge">⚙️ Action: $1</div>')
                .replace(/\\*\\*Observation:\\*\\*[\\s\\S]*?```([\\s\\S]*?)```/g, '<div style="background:#1a1b26;padding:8px;border-radius:6px;font-size:12px;color:#a9b1d6;margin-top:5px;max-height:100px;overflow:auto;">$1</div>');
      ...
      renderBub(bubble, full, false); 
      // Re-apply formatter for final state
      bubble.innerHTML = renderMd(full)
        .replace(/\\*\\s*\\(Executing Tool: (.*?)\\)\\s*\\*/g, '<div class="fbadge">⚙️ Action: $1</div>')
        .replace(/\\*\\*Observation:\\*\\*[\\s\\S]*?```([\\s\\S]*?)```/g, '<div style="background:#1a1b26;padding:8px;border-radius:6px;font-size:12px;color:#a9b1d6;margin-top:5px;max-height:100px;overflow:auto;">$1</div>');
      addRxns(mDiv, full); stpBtn.classList.remove('on'); abortCtl = null;
    }
